...
-- Demo check-ins run 30 days so the board stays alive for testing (the sweep
-- leaves active check-ins alone). Re-runnable: fixed ids, on conflict skip.
insert into public.trip_patterns
  (id, user_id, mode, route_or_line, direction, days_of_week, window_start, window_end, station_or_code, one_off)
select v.id::uuid, v.user_id::uuid, v.mode, v.route, v.dir, v.days::int[], v.ws, v.we, v.code, v.one_off
from (values
  ('22222222-2222-4222-8222-222222222201','11111111-1111-4111-8111-111111111106','place','Gate B lounge (BWI)','Regular','{1,2,3,4,5}','15:00','20:00','BWI',false),
  ('22222222-2222-4222-8222-222222222202','11111111-1111-4111-8111-111111111107','place','Food court (BWI)','Regular','{1,3,5}','11:00','14:00','BWI',false),
  ('22222222-2222-4222-8222-222222222203','11111111-1111-4111-8111-111111111108','place','Amtrak platform (BWI)','Regular','{1,2,3,4,5}','07:00','09:30','BWI',false),
  ('22222222-2222-4222-8222-222222222204','11111111-1111-4111-8111-111111111109','flight','BWI → BOS','One way','{1}','06:00','09:00','BWI',false),
  ('22222222-2222-4222-8222-222222222205','11111111-1111-4111-8111-111111111110','flight','BWI → ATL','One way','{4}','16:00','19:00','BWI',false),
  ('22222222-2222-4222-8222-222222222206','11111111-1111-4111-8111-111111111111','place','Passing through (BWI)','Regular','{2}','12:00','23:59','BWI',true),
  ('22222222-2222-4222-8222-222222222207','11111111-1111-4111-8111-111111111112','place','Layover (BWI)','Regular','{2}','13:00','23:59','BWI',true)
) as v(id, user_id, mode, route, dir, days, ws, we, code, one_off)
join public.profiles p on p.id = v.user_id::uuid
...
select
  (select count(*) from public.trip_patterns where station_or_code = 'BWI') as bwi_patterns,       -- want 7+
  (select count(*) from public.trip_patterns where one_off) as one_off_patterns,                    -- want 2+
  (select count(distinct rkey) from public.trip_patterns where mode='place' and station_or_code='BWI') as bwi_place_keys, -- want 1 (venue-wide!)
